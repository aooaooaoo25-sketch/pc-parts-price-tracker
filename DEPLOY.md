...
3. 部署：
   ```powershell
   .\deploy.ps1 -ProjectName pc-price-tracker
   ```
   首次會提示建立專案，照著選即可。之後每次跑這行就會更新。
...
本機每天爬完會更新 `report.json`。要讓線上也每天更新，在爬蟲排程後接著部署：

- 簡單做法：在 `crawl_daily.ps1` 最後加一行 `& "$PSScriptRoot\deploy.ps1" -ProjectName pc-price-tracker`
- 排程要**非互動**部署 → 不能用 `wrangler login`，改用 API Token：
  1. Cloudflare → My Profile → API Tokens → 建 Token（範本「Edit Cloudflare Pages」）
  2. 設環境變數（系統層級，供排程讀取）：`CLOUDFLARE_API_TOKEN`、`CLOUDFLARE_ACCOUNT_ID`
  3. 之後 `npx wrangler pages deploy` 會自動用 Token，不需登入
